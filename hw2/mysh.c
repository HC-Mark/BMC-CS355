...
cir_buf history;
bool gate = false; //use to test whether we want to check ctrl-c
/*= {
  .size = HISTSIZE,
...
void mysh_loop() {
  ...
  do{
    printf("(╯‵□′)╯︵┻━┻:");
    line = mysh_read_line();
    //we deal with signal ctrl-c and ctrl-z here
    if(gate) {
      free(line);
      gate = false;
      continue;
    ...
    */
    ...
    //if we input controlc c in cat -, we will detect it here
    if(gate) {
      printf("in control c\n");
      free(line);
      free(arguments);
...
int mysh_builtin (char** args) {
  ...
  if(checked[0] == '!') {
    ...
    history.head = history.head - 1;
    char* free_item;
    cir_buf_get_head(&history,&free_item); //can not free the free_item here,otherwise, can not compare
    ...
    printf("checked is %s\n",args[0]);
    if(strcmp(checked, "!!") == 0) {
      //after the strcmp, we know the exact command, so able to free here
      free(free_item);
      if(history.head == 0) {
	...
	fprintf(stderr,"number is too large or too small, can't store that many commands in data base.");
      ...
      if(tmp_args[0] != NULL)
	free(tmp_args);
      //here we don't need know the content of check, free it
      free(free_item);
      return tmp_status;
    }
